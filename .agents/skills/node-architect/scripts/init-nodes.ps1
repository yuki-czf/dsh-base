...
Copy-Item $protocol (Join-Path $nodesDir 'PROTOCOL.md') -Force
...
- **存档时机**：节点完成 / 用户说"存档、这节点差不多了" / 大改动落地 / 会话收尾 → 按协议执行五步存档（PROGRESS → SESSIONS → DECISIONS → CONTEXT → archive），锁与校验优先用 skill 的 `scripts/save.ps1`。
- **压缩即恢复**：感知到上下文被压缩或记忆断裂 → 立即重读 `.nodes/CONTEXT.md`、`PROGRESS.md` 恢复状态再继续。
- **并发纪律**：DECISIONS 只追加；PROGRESS 只改自己节点的行；写 CONTEXT 前后用 `save.ps1 lock/unlock`（10 分钟超时）。
