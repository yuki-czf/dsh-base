...
1. **开工侦察（登记后、写代码前必答；结论落入 `plans/<节点名>.md`，无批次计划的小节点落入 CONTEXT「下一步」）**：
...
3. **批内检查点（未拆批的中型节点必守）**：预估 15~40 分钟、不拆批的节点，每完成一个子块（一个视图 / 一个模块 / 一组同类改动）即跑一次 `save.ps1 save` 中途存档——巨头会话被切成有干净恢复点的段，返工循环最多污染一段，崩溃/压缩随时可续。
