...
2. **节点先行**：任何长于一次会话的工作，先在 `PROGRESS.md` 登记节点（名称 + 验收标准 + owner 会话），并把下一步的设计要点写入 `CONTEXT.md` 的"下一步"，再写代码。地图先于道路。登记后、写代码前完成**开工侦察**（测试基建盘点 / 新依赖打样 / 门禁合并；结论用 `save checkpoint -Recon` 登记，verify 对缺失者 WARN——见 PROTOCOL「执行效率协议」）。
3. **必须存档的时机**：① 节点验收标准达成；② 用户表达存档意向（"存档 / 这节点差不多了 / 先到这"）；③ 完成大结构改动；④ 出现会话即将结束的迹象。存档走【存档流程】。
4. **体积纪律（v1.7.0 硬闸门）**：`CONTEXT.md` ≤60 行且每行 ≤6000 字符；`PROGRESS.md` 表格行 ≤600 字符；`DECISIONS.md` ≤200 行。超限时 `save`/`commit` 被 **exit 4 拦截**（10 分钟内同命令重跑放行一次）——按提示跑 `trim-context` / `trim-progress` / `trim-decisions` 滚出到 `archive/` 后重存。细节一律写 `archive/<节点>.md`，核心文件只留一句话状态。
...
预判超单会话预算（上下文将超压缩点，或工作量：预估 >15 文件 / >100 处改动 / >40 分钟）→ 先问用户选 **spec-superflow**（重型）还是**本分批约定**（轻量），二选一不混用。执行纪律与命令详见 `.nodes/PROTOCOL.md`「批次协议」节和 `references/QUICKREF.md`。
